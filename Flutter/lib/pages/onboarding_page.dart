...
class _OnboardingPageState extends State<OnboardingPage> {
  ...
  // Step 3: Budget bulanan
  final TextEditingController _budgetCtrl = TextEditingController();

  // Step 4: Frekuensi makan per hari
  int _mealFreq = 3;

  bool _isLoading = false;

  // Sementara hardcoded user ID (ganti dengan ID dari auth nanti)
  final String _userId = '6a0befd258363027f1628ca0';
  ...
  void _toggleFav(String item) {
    setState(() {
      if (_favFoods.contains(item)) {
        _favFoods.remove(item);
      } else {
        _favFoods.add(item);
      }
    });
  ...
  Future<void> _submitOnboarding() async {
    ...
    final budgetRaw = _budgetCtrl.text.replaceAll('.', '').replaceAll(',', '');
    final budget = double.tryParse(budgetRaw) ?? 0;

    final body = {
      'kategori_favorit': _favFoods,
      'alergi': _allergies,
    ...
    try {
      ...
      final response = await http.put(
        Uri.parse('${ApiService.baseUrl}/users/onboarding/$_userId'),
        headers: {
      ...
      if (response.statusCode == 200 && json['success'] == true) {
        ...
        _showErrorSnackbar(
            json['message'] ?? 'Gagal menyimpan data onboarding.');
      }
    } catch (e) {
      if (mounted) _showErrorSnackbar('Koneksi gagal. Periksa jaringanmu.');
    } finally {
              ...
              AnimatedFractionallySizedBox(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [
                      BoxShadow(
                          color: AppTheme.orange500.withOpacity(0.4),
                          blurRadius: 6)
                    ],
                  ),
                ),
  ...
  Widget _buildStep() {
    switch (_step) {
      case 1:
        return _StepFoodPreferences(selected: _favFoods, onToggle: _toggleFav);
      case 2:
...
class _StepFoodPreferences extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const groups = [
      {
        'label': 'Rasa',
        'tags': ['Pedas', 'Manis', 'Gurih', 'Asin', 'Asam', 'Umami']
      },
      {
        'label': 'Bahan Utama',
        'tags': [
      ...
      {
        'label': 'Jenis Masakan',
        'tags': ['Nasi', 'Mie', 'Sup/Soto', 'Gorengan', 'Bakar/Panggang']
      },
    ];
    ...
    return Column(
      ...
      children: [
        ...
        Text(
            'Pilih minimal 1. CookCash+ akan merekomendasikan menu sesuai seleramu.',
            style: TextStyle(
                fontSize: 13,
        ...
        ...groups.map((g) => Column(
              ...
              children: [
                ...
                Wrap(
                  spacing: 8,
                  runSpacing: 8,
                  children: (g['tags'] as List<String>).map((tag) {
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: isSel
                              ? [
                                  BoxShadow(
                                      color:
                                          AppTheme.orange500.withOpacity(0.25),
                                      blurRadius: 8,
    ...
    return Column(
      ...
      children: [
        ...
        const Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Text('Ada Alergi Bahan Makanan? ⚠️',
                style: TextStyle(
                    fontSize: 24,
                    fontWeight: FontWeight.w900,
                    color: AppTheme.slate900,
        ...
        if (!noAllergy) ...[
          ...
          if (selected.any((s) => !presets.contains(s))) ...[
            ...
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: selected.where((s) => !presets.contains(s)).map((tag) {
                return Container(
                  padding:
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 6),
        Text(
            'Berapa total budget makan kamu dalam sebulan? CookCash+ akan memantau pengeluaran agar kamu tidak defisit.',
            style: TextStyle(
                fontSize: 13,
                color: context.colors.textSecondary,
                fontWeight: FontWeight.w500,
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Divider(color: AppTheme.orange500.withOpacity(0.4), thickness: 2),
            ],
          ),
        ),
        const SizedBox(height: 16),
