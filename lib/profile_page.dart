...
class _ProfilePageState extends State<ProfilePage> {
  ...
  final _allFavFoods = ['Pedas 🌶️', 'Manis 🍬', 'Gurih 🥘', 'Asin 🧂', 'Ayam 🍗', 'Sapi 🥩', 'Seafood 🦐', 'Sayuran 🥦'];
  final _allAllergies = ['Kacang 🥜', 'Susu 🥛', 'Telur 🥚', 'Seafood 🦐', 'Gluten 🍞', 'Kedelai 🌱'];
  final _allTools = ['Kompor 🔥', 'Oven 🎛️', 'Microwave 🍱', 'Blender 🌪️', 'Rice Cooker 🍚', 'Kulkas ❄️'];

  void _toggleItem(String item, List<String> list, Function(List<String>) setter) {
    setState(() {
      if (list.contains(item)) {
        setter(List.from(list)..remove(item));
      } else {
              ...
              delegate: SliverChildListDelegate([
                ...
                _ProfileSection(
                  children: [
                    ...
                    Row(
                      children: [
                        const Icon(Icons.calendar_today_rounded, color: AppTheme.blue500, size: 20),
                        const SizedBox(width: 8),
                        const Text('Siklus Pengaturan Uang', style: TextStyle(fontWeight: FontWeight.w900, color: AppTheme.slate800)),
                      ],
                    ),
                    const SizedBox(height: 12),
                    if (_isEditing)
